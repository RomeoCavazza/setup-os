...
Two runtime scripts drive the dynamic modules. [`WaybarCava.sh`](https://github.com/RomeoCavazza/setup-os/blob/main/config/hypr/waybar/WaybarCava.sh) generates a temporary Cava config on launch with 14 bars at 60fps over PulseAudio, pipes the raw ASCII output through character substitution and silence masking, and outputs unicode bar characters for the `custom/cava` module. [`activeapp.sh`](https://github.com/RomeoCavazza/setup-os/blob/main/config/hypr/waybar/activeapp.sh) queries `hyprctl activewindow` for the focused window's class, maps it to a Nerd Font icon via a case statement (`firefox` → , `code` → 󰨞, `foot` → ), and outputs JSON for the active window module.

---

## 7. Rofi

Rofi is configured in [`config/rofi/`](https://github.com/RomeoCavazza/setup-os/blob/main/config/rofi/) with two active launch paths. The sidebar launcher runs via [`rofi-push.sh`](https://github.com/RomeoCavazza/setup-os/blob/main/config/rofi/scripts/rofi-push.sh), which shifts Hyprland's `gaps_out` to create space for the panel without overlapping windows, then restores original gaps on close. The application grid runs via [`rofi-grid.sh`](https://github.com/RomeoCavazza/setup-os/blob/main/config/rofi/bin/rofi-grid.sh), which temporarily increases blur size and kills Waybar on open, restoring both on exit. The sidebar uses [`column-tco.rasi`](https://github.com/RomeoCavazza/setup-os/blob/main/config/rofi/custom/column-tco.rasi) and the grid uses [`apps-grid.rasi`](https://github.com/RomeoCavazza/setup-os/blob/main/config/rofi/themes/apps-grid.rasi). Both are referenced from [`hyprland.conf`](https://github.com/RomeoCavazza/setup-os/blob/main/config/hypr/hyprland.conf) via the `$menu` and `$powermenu` variables.
