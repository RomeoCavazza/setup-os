...
All services are Nix-declared in [`modules/observability.nix`](https://github.com/RomeoCavazza/nixos-config/blob/main/modules/observability) and activated with `nixos-rebuild`.
...
2. **Compile**: [`grafana-generate`](https://github.com/RomeoCavazza/nixos-config/blob/main/config/bin/grafana-generate) produces the provisioned JSON in [`config/grafana/dashboards/`](https://github.com/RomeoCavazza/nixos-config/blob/main/config/grafana/).
